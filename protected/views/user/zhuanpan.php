...
<body>
	<div class="wrap">
		<img src="http://<?php echo $this->cdn;?>/static/images/bg.jpg?v=0515" alt="" class="bgimg">
		<div class="main">
			<div class="tit">
			</div>
			<div class="ly-plate">
				<div class="rotate-bg">
					<img src="http://<?php echo $this->cdn;?>/static/images/pan.png?v=0515" alt="">
			...
			<div class="rules">
				<p>活动规则：</p>
				<p>花费30微积分获得1次抽奖机会</p>
				<p>奖励兑换：进入游戏—打开设置界面—输入兑换CD-KEY</p>
			</div>
			<div class="record">
			...
			click: function(){
				...
				$.ajax({
					...
					if(data.s == 'ok'){
                    	...
                    	if(data.data.id==2){
                        	rotateFunc(2,300,'恭喜您抽中限量打火机一个')
                        	//rotateFunc(5,240,'恭喜您抽中劳模礼包')
							//$('.tip1').find('.awardPWD').html('领取CDK是：'+data.data.cdkey);
                    	}
                    	if(data.data.id==3){
                        	rotateFunc(5,120,'恭喜您抽中银币礼包')
							$('.tip1').find('.awardPWD').html('领取CDK是：'+data.data.cdkey);
                        	//rotateFunc(5,360,'恭喜您抽中超值礼包')
							//$('.tip1').find('.awardPWD').html('领取CDK是：'+data.data.cdkey);
                    	}
                    	if(data.data.id==4){
                        	rotateFunc(5,180,'恭喜您抽中包子礼包')
							$('.tip1').find('.awardPWD').html('领取CDK是：'+data.data.cdkey);
                    	}
                    	if(data.data.id==5){
                        	rotateFunc(5,240,'恭喜您抽中鸡腿礼包')
							$('.tip1').find('.awardPWD').html('领取CDK是：'+data.data.cdkey);
                    	}	
                    	if(data.data.id==6){
                        	//rotateFunc(6,420,'恭喜获得ipad mini')
                        	rotateFunc(5,360,'恭喜您抽中转盘礼包')
							$('.tip1').find('.awardPWD').html('领取CDK是：'+data.data.cdkey);
                    	}	
					}
				});
